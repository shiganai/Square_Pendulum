...
public class rodrigues_Rotation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ax_Vec_X = matrix.convert_Vector3_To_Matrix(new Vector3(1, 0, 0));
        ax_Vec_Y = matrix.convert_Vector3_To_Matrix(new Vector3(0, 1, 0));
        ax_Vec_Z = matrix.convert_Vector3_To_Matrix(new Vector3(0, 0, 1));

        r_Shoulder = matrix.transpose(new matrix(new float[,]
        {
            { 0.5f, 1, 0 }
        }));
        l_Shoulder = matrix.transpose(new matrix(new float[,]
        {
            { -0.5f, 1, 0 }
        }));
        r_Hip = matrix.transpose(new matrix(new float[,]
        {
            { 0.5f, -1, 0 }
        }));
        l_Hip = matrix.transpose(new matrix(new float[,]
        {
            { -0.5f, -1, 0 }
        }));
    }
    ...
    void Update()
    {
        ...
        //transform.forward = matrix.convert_Matrix_To_Vector3(r_Shoulder - r_Hip);
        //transform.right = matrix.convert_Matrix_To_Vector3(r_Shoulder - l_Shoulder);
    }
